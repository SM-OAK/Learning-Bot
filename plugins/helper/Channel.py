...
async def media(bot, message):
    ...
    # FIXED: Added more MIME types to ensure movies like 'Mirai' are not skipped
    if media.mime_type in ['video/mp4', 'video/x-matroska', 'application/octet-stream', 'video/webm']: 
...
async def send_movie_updates(bot, file_name, file_id):
    ...
    # FIXED: Using ADMINS[0] safely
    admin_id = ADMINS[0] if ADMINS else "bot"
    btn = [
        [InlineKeyboardButton('Get File', url=f'https://t.me/{temp.U_NAME}?start=pm_mode_file_{admin_id}_{file_id}')]
    ]
